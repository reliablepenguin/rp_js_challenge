...
1. **Gate (pre-app):** A tiny PHP file (`sec_js_gate.php`) runs for every PHP request via `auto_prepend_file`. If the short-lived cookie is absent/invalid, it sends a 302 to `/js-check.php?b=<original URL>` and exits immediately. Drupal is not loaded.
...
* **Bypasses:** Static assets, admin/login, cron, and the challenge endpoints are always allowed to avoid breaking admin or caching.
...
// /priv/sec_js_gate.php — lightweight gate with crawler allowlist
...
5. **Expiry test:** Wait 10+ minutes or delete cookie → next request re-challenges.
6. **Static assets unaffected:** CSS/JS/images load without being gated.
7. **Rate-limit smoke (if enabled):** Abnormally fast requests to `/user/login` → `429` as expected.
